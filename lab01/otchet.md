...
1. Преподаватель нажимает «Рассчитать»
2. MainForm создаёт ExamResultBuilder
3. Трижды вызывается ProcessTopic() — по одному разу на каждую тему
4. Вызывается Build() → создаётся ExamResult
5. Если GoesToExam == false — сообщение «автомат», конец
6. Если true — QuestionHelper загружает вопросы из QuestionStorage и возвращает случайные по нужным темам
7. Вопросы отображаются преподавателю
