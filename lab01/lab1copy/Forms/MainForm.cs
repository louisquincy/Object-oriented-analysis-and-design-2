       ...
       private void BtnCalculate_Click(object sender, EventArgs e)
        {
            ...
            {
                var overdueInTopic = _labWorks
                    .Where(l => l.TopicNumber == topic && l.IsOverdue)
                    .ToList();

                if (overdueInTopic.Any())
                {
                    topicsForExam.Add(topic);
                    overdueCount += overdueInTopic.Count;
                }
            }

            bool goesToExam = overdueCount > 0;
            _lastExamResult = new ExamResult(goesToExam, topicsForExam, overdueCount);
